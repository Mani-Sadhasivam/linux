...
{
	gpiod_set_value(ctx->resx_gpio, 1);
...
static int tc9563_pwrctrl_bring_up(struct tc9563_pwrctrl_ctx *ctx)
{
	...
	gpiod_set_value(ctx->resx_gpio, 0);
...
static int tc9563_pwrctrl_probe(struct platform_device *pdev)
{
	...
	ctx->resx_gpio = devm_gpiod_get(dev, "resx", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->resx_gpio)) {
		ret = dev_err_probe(dev, PTR_ERR(ctx->resx_gpio), "failed to get RESX GPIO\n");
		goto remove_i2c;
	}
