...
#define DMA_MODE_CFE				BIT(29)
#define DMA_MODE_LME				BIT(30)
#define DMA_MODE_CME				BIT(31)
...
#define DMA_LLC_SUSPEND				BIT(16)

/* DMAX_INT_CTL */
#define DMA_INTCTL_BLOCK			BIT(0)
#define DMA_INTCTL_SUPER_BLOCK			BIT(1)
#define DMA_INTCTL_FRAME			BIT(2)
#define DMA_INTCTL_HALF_FRAME			BIT(3)
#define DMA_INTCTL_LAST_FRAME			BIT(4)

/* DMAX_INT_STATUS */
#define DMA_INTSTAT_BLOCK			BIT(0)
#define DMA_INTSTAT_SUPER_BLOCK			BIT(1)
#define DMA_INTSTAT_FRAME			BIT(2)
#define DMA_INTSTAT_HALF_FRAME			BIT(3)
#define DMA_INTSTAT_LAST_FRAME			BIT(4)
					   ...
					   struct owl_dma_lli *next)
{
	...
	if (prev) {
		...
		prev->hw.ctrla |= llc_hw_ctrla(DMA_MODE_LME, 0);
	}
			...
			u32 len, enum dma_transfer_direction dir)
{
	...
	hw->next_lli = 0;   /* 1 link list by default */
	hw->saddr = src;
	hw->daddr = dst;
	...
	hw->ctrlb = llc_hw_ctrlb(DMA_INTCTL_SUPER_BLOCK);
				    ...
				    struct owl_dma_pchan *pchan)
{
	...
	pchan_update(pchan->base + DMAX_INT_STATUS, 0xff, false);

	spin_lock_irqsave(&od->lock, flags);
	dma_update(od->base + DMA_IRQ_EN0, (1 << pchan->id), false);
...
static int owl_dma_start_next_txd(struct owl_dma_vchan *vchan)
{
	...
	dma_update(od->base + DMA_IRQ_EN0, (1 << pchan->id), true);
...
static irqreturn_t owl_dma_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < od->nr_pchans; i++) {
		...
		if (chan_irq_pending && !(global_irq_pending & BIT(i)))	{
			...
			/* clear IRQ status for this pchan */
			pchan_update(pchan->base + DMAX_INT_STATUS,
					0xff, false);
